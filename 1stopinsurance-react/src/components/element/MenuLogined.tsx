...
<<<<<<< HEAD
// แก้ไข: นำเข้า Link และ useNavigate จาก react-router-dom
import { Link, useNavigate } from 'react-router-dom'; 
// Icons จาก lucide-react ยังคงใช้ได้
import { Bell, ChevronDown, UserCircle, LogOut, Menu, X } from 'lucide-react';
// แก้ไข: ลบการนำเข้า next/image
...
const navLinks = [
    // แก้ไข href ให้เป็น path ที่ใช้ใน react-router-dom
    { href: "/car-insurance-form", label: "ประกันรถยนต์" }, 
    { href: "/about", label: "เกี่ยวกับเรา" },
    { href: "#footer", label: "ติดต่อเรา" }, // Anchor link ยังใช้ได้ปกติ
];

// ลบ "use client" ออก
export default function MenuLogined({ activePage }: AdminHeaderProps) {
    // แก้ไข: ใช้ useNavigate สำหรับการเปลี่ยนหน้าด้วยฟังก์ชัน
    ...
    const logout = () => {
        localStorage.clear();
        setIsMenuOpen(false); 
        // แก้ไข: ใช้ navigate() แทน window.location.assign() เพื่อให้ react-router-dom จัดการ
        navigate("/customer/login"); 
    };

    // การอ่าน localStorage ยังคงใช้ได้ใน Client-side Component
    ...
    // ตรวจสอบว่ามี customerData เพื่อแสดงผล
    if (!customerData) {
        // กรณีไม่มีผู้ใช้เข้าสู่ระบบ (ปรับปรุงส่วนนี้เล็กน้อยเพื่อให้ใช้ Link จาก react-router-dom)
        return (
            <header className="flex justify-between items-center px-8 py-4 bg-white shadow-sm cursor-pointer" style={{zIndex:9999}}>
                ...
                <nav className="hidden md:flex space-x-6 text-gray-700 font-medium">
                    ...
                    <Link to="#footer" className="hover:text-blue-700">ติดต่อเรา</Link>
                </nav>

                <Link to="/customer/login" // แก้ไข: ใช้ Link to={...}
    ...
    // กรณีมีผู้ใช้เข้าสู่ระบบ
    return (
        // ลบเงื่อนไข && ( ) ครอบด้านนอก
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Link to="/customer/mainpage" className="flex items-center gap-2">
                            ...
                            <img
                                src="/fotos/Logo.png"
                                alt="logo"
                                // ลบ width/height ออก หรือกำหนดเป็น string ใน img tag
                                className="h-10 w-auto"
                                onError={(e) => {
            ...
            <div className="flex items-center gap-5">
                ...
                <div className="relative" ref={menuRef}>
                    <button
                        ...
                        className="flex items-center gap-2 text-slate-700 hover:bg-slate-100 p-2 rounded-full transition-colors"
                    >
>>>>>>> 91b1a25cae140de39b0345b88fce36bfe8c72c9c
...
<<<<<<< HEAD
                        ...
                        <div className="cursor-pointer absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2 animate-fade-in-down z-50">
                            <div className="border-b">
                                {/* แก้ไข: ใช้ Link to={...} */}
                                <Link to={"/customer/profile"} className="flex items-center gap-3 w-full px-4 py-2 text-sm  hover:bg-blue-50" onClick={() => setIsMenuOpen(false)}>
                                    <UserCircle size={28} className="text-blue-800 " />
                    ...
                    <div className="flex flex-col p-3 space-y-2">
                        {navLinks.map((link) => (
                            <Link
                                key={link.href}
                                to={link.href} // แก้ไข: ใช้ to={...}
                                onClick={() => setIsNavOpen(false)} // ปิดเมนูเมื่อกด
            ...
            {/* Animation (สามารถคงไว้ได้ แต่ควรย้ายไปไว้ในไฟล์ CSS หรือใช้ Tailwind Utility) */}
